...
{
    [self storeImage:image imageData:imageData forKey:key toDisk:toDisk toMemory:YES];
}
    ...
    dispatch_async(self.ioQueue, ^
                   {
                       ...
                       //NSLog(@"Fetched %@, it was %@", key, diskImage);
                       if (diskImage)
                       {
...
{
    [self queryDiskCacheForKey:key diskOnly:FALSE done:doneBlock];
}
